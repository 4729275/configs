...
apt-get install audacity curl eza fastfetch ffmpeg fonts-roboto gimp gnome-calendar gnome-console gnome-snapshot gnome-tweaks gnome-weather handbrake howdy htop inkscape kid3 kleopatra libpam-python libreoffice mkvtoolnix-gui nextcloud-desktop obs-studio psensor python3-dlib python3-tk rhythmbox scdaemon systemd-zram-generator texstudio tilem timeshift tlp tlp-rdw v4l-utils vim virt-manager vlc wireguard xournalpp yt-dlp -y
apt-get remove fonts-noto* -y
apt-get install fonts-noto-cjk fonts-noto-color-emoji -y
snap install chromium zoom-client
